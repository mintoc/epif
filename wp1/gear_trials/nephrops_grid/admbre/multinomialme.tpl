...
// Run with ./multinomialme -l1 50000000 -l2 200000000 -l3 50000000
//-------------------------------------------------------------
...
PARAMETER_SECTION
  ...
  init_bounded_vector a(1,nchol,-1.0,1.0,3)   // Free parameters in C
  //init_bounded_vector a(1,nchol,-1.0,1.0,-1)   // Free parameters in C
  // random effects
...
PROCEDURE_SECTION
  ...
  for (int i=1;i<=n;i++){
    for (int j=1;j<=m;j++){
      P(i,j) = exp(eta(i,j))/rowsums(i,1) + 1.e-12; // see if this helps the log of very small values
    }
  }
  // log-likelihood for the observations
  for (int i=1;i<=n;i++){
      nll += -1.0 * (Y(i) * log(P(i)) + gammln(sum(Y(i)) + 1.0) - sum(gammln(Y(i) + 1.0)));
  }
