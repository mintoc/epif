...
setwd("C:/Users/Coilin/Documents/docs/analyses/epif/wp1/gear_trials/nephrops_grid/R")
...
##
## write.csv(subset(grid.neph.cast, fHAUL == "H1"), file = "../data/haul1.csv", row.names = FALSE)
...
unique(grid.neph.dat[, c("fHAUL", "COMPARTMENT", "SUBSRATIO")])
...
prop.compartment.df <- data.frame(
						...
						fHAUL = rep(grid.neph.cast$fHAUL, times = 4)
                         )
...
rownames(pred.p) <- c(unique(grid.neph.cast$netconfig), "Overall")
