...
vars2keep <- c("Haul.No", "Codend.No", "Mesh.Size", "Net.position", "Carapace.length", "Count", "SUBSRATIO","Total.catch")
...
##This loop is used to fill the Total.catch column for all nets
for(i in 1:dim(neph.cast)[1]){
  haul.dat <- subset(neph.cast, Haul.No == neph.cast$Haul.No[i])
  
  if(is.na(neph.cast$ms100mm_Total.catch[i])){
    neph.cast$ms100mm_Total.catch[i] <- unique(na.omit(haul.dat$ms100mm_Total.catch))
  }
  
  if(is.na(neph.cast$ms70mm_Total.catch[i])){
    neph.cast$ms70mm_Total.catch[i] <- unique(na.omit(haul.dat$ms70mm_Total.catch))
  }
  
  if(is.na(neph.cast$ms80mm_Total.catch[i])){
    neph.cast$ms80mm_Total.catch[i] <- unique(na.omit(haul.dat$ms80mm_Total.catch))
  }
  
  if(is.na(neph.cast$ms90mm_Total.catch[i])){
    neph.cast$ms90mm_Total.catch[i] <- unique(na.omit(haul.dat$ms90mm_Total.catch))
  }
}  

##Created Variables for Carapace Length
ncl <- neph.cast$Carapace.length ##/ (2*sd( neph.cast$Carapace.length))
ncl2 <- neph.cast$Carapace.length^2
ncl3 <- neph.cast$Carapace.length^3

##Created Variable for Weight
catch <- with(neph.cast, cbind(ms100mm_Total.catch, ms90mm_Total.catch, ms80mm_Total.catch, ms70mm_Total.catch))

##Set up model
##Model 1 - Just carapace length
...
fit1 <- optim(par=start.par, Y=dvar, X=invar, off=offset.mat, fn=dm.nll.o, control = list(trace = 1), method="BFGS")
...
##Model 2 - Addition of Carapace length squared
invar2 <- cbind(1, ncl, ncl2) 
npar <- ncol(dvar) * ncol(invar2)
start.par <- rep(0,npar)

fit2 <- optim(par=start.par, Y=dvar, X=invar2, off=offset.mat, fn=dm.nll.o, control = list(trace=1, reltol=1e-16, abstol=1e-16), method="BFGS")

beta2 <- matrix(fit2$par, ncol=ncol(dvar))
eta <- invar2 %*% beta2

p <- exp(eta)/rowSums(exp(eta))

##Model 3 - Addition of Weight (no carapace length squared)
invar3 <- cbind(1, ncl, catch)
npar <- ncol(dvar)*ncol(invar3)
start.par <- rep(0, npar)

fit3 <- optim(par=start.par, Y=dvar, X=invar3, off=offset.mat, fn=dm.nll.o, control = list(trace=1, reltol=1e-16, abstol=1e-16), method="BFGS")

beta3 <- matrix(fit3$par, ncol=ncol(dvar))
eta <- invar3 %*% beta3
