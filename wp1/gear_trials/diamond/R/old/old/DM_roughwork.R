...
Y <- t(rmultinom(100, size=50, prob=c(0.4, 0.2, 0.3, 0.1)))
...
para <- ncol(Y) * ncol(X)
st.par<- rep(0, para)
par.upr <- rep(100, para)
par.lwr <- rep(-10, para)

off.mat <- cbind(rep(0.125, nrow(Y)), rep(0.25,nrow(Y)),rep(0.1,nrow(Y)),rep(0.5, nrow(Y)))

fit <- optim(par = st.par, Y = Y, X = X, off = off.mat, control = list(trace=1), fn = dm.nll.o, method = "L-BFGS-B", lower = par.lwr, upper = par.upr)
...
fit <- optim(par = st.par, Y = Y, X = X, off = off.mat, control = list(trace=1), fn = dm.nll.o, method = "BFGS")
...
ddm <- function(n, alpha, log = FALSE){
  ...
  lpdf <- lgamma(sum(alpha)) - lgamma(sum(alpha + n)) + sum(lgamma(alpha + n) - lgamma(alpha))
  ##lpdf <- lgamma(sum(alpha)) - lgamma(sum(alpha + n)) + sum(lgamma(alpha + n) - lgamma(alpha))
  pdf <- exp(lpdf)
  if(log){
    return(lpdf)
