...
## make the data same format as "Fish" dataset

names(our.lass.neph.cast)[names(our.lass.neph.cast) == "m70mm_Total.catch"] <- "tc.m70"
names(our.lass.neph.cast)[names(our.lass.neph.cast) == "m80mm_Total.catch"] <- "tc.m80"

m70.dat <- our.lass.neph.cast[, c("m70mm_Count", "Carapace.length", "tc.m70", "tc.m80")]

m70.dat$mesh <- "m70"
m70.dat <- m70.dat[rep(1:nrow(m70.dat), times = m70.dat$m70mm_Count), ]

m80.dat <- our.lass.neph.cast[, c("m80mm_Count", "Carapace.length", "tc.m70", "tc.m80")]
m80.dat$mesh <- "m80"
m80.dat <- m80.dat[rep(1:nrow(m80.dat), times = m80.dat$m80mm_Count), ]

vars2keep <- c("mesh", "Carapace.length", "tc.m70", "tc.m80")

m.dat <- rbind(m70.dat[, vars2keep], m80.dat[, vars2keep])

names(m.dat)[names(m.dat) == "Carapace.length"] <- "cl"

rownames(m.dat) <- NULL

mlogit.dat <- mlogit.data(m.dat, varying = c(3:4), shape = "wide", choice = "mesh")

## multinomial
fit <- mlogit(mesh ~ 0 | 1, data = mlogit.dat)
