...
png("../tex/figures/Figure2.png", height = 15, width = 17, units = "cm", res = 500)
prop.plot
dev.off()
...
png("../tex/figures/Figure3.png", height = 15, width = 17, units = "cm", res = 500)
...
png("../tex/figures/Figure5.png", height = 15, width = 17, units = "cm", res = 500)
##png("../tex/figures/bubble_gum_plot_overall_with_fit_v0.png", height = 7, width = 8, units = "in", res = 400)
prop.plot.nocolour +
  ...
  ##geom_line(data = plot.pred.df.admb, aes(y = dm.proportion), colour = "blue")
dev.off()

## predictions at the haul level with random effects
...
eta.hat <- X %*% beta.hat + betacond.hat * Xcond + cbind(0, uhat)[gps,]

P.hat <- exp(eta.hat) / rowSums(exp(eta.hat))

## predictions at haul level for model with no random effects
coef.admb.nore <- read.table("../admb/conlogit/conditional.std", header = TRUE)

beta.hat.nore <- cbind(0, matrix(subset(coef.admb.nore, name == "beta0")$value, nrow = p, byrow = TRUE))

betacond.hat.nore <- subset(coef.admb.nore, name == "betacond")$value

eta.hat.nore <- X %*% beta.hat.nore + betacond.hat.nore * Xcond 
...
## predictions for different net configurations
eta <- cbind(1, 30, 0, 0, 0) %*% beta.hat
round(exp(eta)/sum(exp(eta)), 3)

## some of the haul fits look low?

## SANDBOX - effect of weight

##eta <- Xpred[1:2,]%*%beta.hat + betacond.hat * (Xcondpred[1:2,] - 200)
eta <- Xpred[1:2,]%*%beta.hat + betacond.hat * (Xcondpred[1:2,] * 0)
