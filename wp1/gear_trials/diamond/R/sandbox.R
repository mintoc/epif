...
p.df <- data.frame(weight = c(weights), p = c(p), group = rep(1:3, each = 100))

library(ggplot2)
ggplot(p.df, aes(x = weight, y = p)) + geom_point() + facet_wrap(~group, ncol = 2)
...
## works much better and hessian is invertible
