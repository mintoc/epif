...
names(weight.cast)[grep("^[0-9]", names(weight.cast))] <- paste("mesh.", names(weight.cast)[grep("^[0-9]", names(weight.cast))], sep = "")
...
mnom.length.matrix <- multinom(as.matrix(neph.dat3[, c("70mm_COUNT", "80mm_COUNT", "90mm_COUNT", "100mm_COUNT")]) ~ 
...
plot(neph.dat3[, c("mesh.70mm_Total.Weight", "mesh.80mm_Total.Weight", "mesh.90mm_Total.Weight", "mesh.100mm_Total.Weight")])

## very strong correlation in the counts
round(cor(neph.dat3[, c("mesh.70mm_Total.Weight", "mesh.80mm_Total.Weight", "mesh.90mm_Total.Weight", "mesh.100mm_Total.Weight")]), 3)

## resids versus weights
...
neph.dat3$av.Total.Weight <- apply(neph.dat3[, c("mesh.70mm_Total.Weight", "mesh.80mm_Total.Weight", "mesh.90mm_Total.Weight", "mesh.100mm_Total.Weight")], 1, mean)

mean.wt <- mean(neph.dat3$av.Total.Weight)

neph.dat3$av.Total.Weight.scaled <- neph.dat3$av.Total.Weight - mean.wt

mnom.length.bulk <- multinom(as.matrix(neph.dat3[, c("70mm_COUNT", "80mm_COUNT", "90mm_COUNT", "100mm_COUNT")]) ~ 
...
mnom.length.bulkc <- multinom(as.matrix(neph.dat3[, c("70mm_COUNT", "80mm_COUNT", "90mm_COUNT", "100mm_COUNT")]) ~ 
                              prop.Carapace.Length + 
                              prop.Carapace.Length2 + 
                              prop.Carapace.Length3 +
                              ##
...
AIC(mnom.length.matrix, mnom.length.bulk, mnom.length.bulk.order1, mnom.length.bulk.order2, mnom.length.bulk.order3, mnom.length.bulk.order3c, mnom.length.bulk.order3.nl)
...
<<eval = TRUE, fig.allign = "center", fig.cap = "Predicted influence of the bulk catch weight (low: 26kg, medium: 60.5kg, high: 120kg) on the proportion of Nephrops catch retained per net. Null hypothesis of equal retention is displayed as the dashed line at 0.25. Minimum landing size of 25mm is shown as the vertical dashed line.", fig.width = 8, fig.height = 7>>=
...
count.array <- with(neph.dat.bulk, tapply(rCOUNT, list(cTotal.Weight, Carapace.Length, Mesh.Size), sum))
...
prop.df$Bulk.Weight <- ifelse(prop.df$Total.Weight == "(0,44]", "Low (26kg)",
                              ifelse(prop.df$Total.Weight == "(44,90]", "Medium (60.5kg)", "High (120kg)")
                              )
