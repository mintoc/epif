...
\hlstd{p70mm} \hlkwb{<-} \hlkwd{plot.lfreq}\hlstd{(}\hlkwc{data} \hlstd{= neph.70mm,} \hlkwc{title.string} \hlstd{=} \hlstr{"70mm"}\hlstd{)}
\hlstd{p80mm} \hlkwb{<-} \hlkwd{plot.lfreq}\hlstd{(}\hlkwc{data} \hlstd{= neph.80mm,} \hlkwc{title.string} \hlstd{=} \hlstr{"80mm"}\hlstd{)}
\hlstd{p90mm} \hlkwb{<-} \hlkwd{plot.lfreq}\hlstd{(}\hlkwc{data} \hlstd{= neph.90mm,} \hlkwc{title.string} \hlstd{=} \hlstr{"90mm"}\hlstd{)}
\hlstd{p100mm} \hlkwb{<-} \hlkwd{plot.lfreq}\hlstd{(}\hlkwc{data} \hlstd{= neph.100mm,} \hlkwc{title.string} \hlstd{=} \hlstr{"100mm"}\hlstd{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{lm0} \hlkwb{<-} \hlkwd{lm}\hlstd{(Carapace.Length} \hlopt{~} \hlnum{1}\hlstd{,} \hlkwc{data} \hlstd{= neph.dat3)}
...
## 
## Call:
## lm(formula = Carapace.Length ~ 1, data = neph.dat3)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
## -20.698  -2.698  -0.698   3.302  53.302 
## 
## Coefficients:
##              Estimate Std. Error t value Pr(>|t|)    
## (Intercept) 28.698050   0.009518    3015   <2e-16 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 4.918 on 266990 degrees of freedom
\end{verbatim}
\begin{alltt}
\hlcom{## predict the density}
\hlstd{pred.df} \hlkwb{<-} \hlkwd{data.frame}\hlstd{(}\hlkwc{Carapace.Length} \hlstd{=} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{100}\hlstd{))}
\hlstd{pred.df}\hlopt{$}\hlstd{dens} \hlkwb{<-} \hlkwd{dnorm}\hlstd{(pred.df}\hlopt{$}\hlstd{Carapace.Length,} \hlkwc{mean} \hlstd{=} \hlkwd{coef}\hlstd{(lm0)[}\hlnum{1}\hlstd{],} \hlkwc{sd} \hlstd{=} \hlkwd{summary}\hlstd{(lm0)}\hlopt{$}\hlstd{sigma)}

\hlcom{## plot the distributional fit}
\hlstd{p} \hlkwb{<-} \hlkwd{ggplot}\hlstd{(neph.dat3,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= Carapace.Length))} \hlopt{+} \hlkwd{geom_histogram}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{y} \hlstd{= ..density..),}
    \hlkwc{fill} \hlstd{=} \hlstr{"slategrey"}\hlstd{,} \hlkwc{color} \hlstd{=} \hlstr{"white"}\hlstd{,} \hlkwc{binwidth} \hlstd{=} \hlnum{1}\hlstd{)} \hlopt{+} \hlkwd{xlim}\hlstd{(}\hlnum{10}\hlstd{,} \hlnum{50}\hlstd{)}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{lm1} \hlkwb{<-} \hlkwd{lm}\hlstd{(Carapace.Length} \hlopt{~} \hlstd{Mesh.Size,} \hlkwc{data} \hlstd{= neph.dat3)}
...
## 
## Call:
## lm(formula = Carapace.Length ~ Mesh.Size, data = neph.dat3)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
## -20.753  -3.303  -0.340   2.697  52.660 
## 
## Coefficients:
##               Estimate Std. Error t value Pr(>|t|)    
## (Intercept)   29.33990    0.01985 1478.23   <2e-16 ***
## Mesh.Size70mm -0.86420    0.02690  -32.13   <2e-16 ***
## Mesh.Size80mm -1.03668    0.02741  -37.82   <2e-16 ***
## Mesh.Size90mm -0.58708    0.02756  -21.30   <2e-16 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 4.903 on 266987 degrees of freedom
## Multiple R-squared:  0.006105,	Adjusted R-squared:  0.006094 
## F-statistic: 546.7 on 3 and 266987 DF,  p-value: < 2.2e-16
\end{verbatim}
\begin{alltt}
\hlcom{## predict the densities}
\hlstd{pred.df} \hlkwb{<-} \hlkwd{expand.grid}\hlstd{(}\hlkwc{Carapace.Length} \hlstd{=} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{100}\hlstd{),} \hlkwc{Mesh.Size} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"70mm"}\hlstd{,}
    \hlstr{"80mm"}\hlstd{,} \hlstr{"90mm"}\hlstd{,} \hlstr{"100mm"}\hlstd{))}

\hlstd{pred.df}\hlopt{$}\hlstd{dens} \hlkwb{<-} \hlkwd{dnorm}\hlstd{(pred.df}\hlopt{$}\hlstd{Carapace.Length,} \hlkwc{mean} \hlstd{=} \hlkwd{predict}\hlstd{(lm1,} \hlkwc{newdata} \hlstd{= pred.df),}
    \hlkwc{sd} \hlstd{=} \hlkwd{summary}\hlstd{(lm1)}\hlopt{$}\hlstd{sigma)}

\hlcom{## plot the distributional fit}
\hlstd{p} \hlkwb{<-} \hlkwd{ggplot}\hlstd{(neph.dat3,} \hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= Carapace.Length))} \hlopt{+} \hlkwd{geom_histogram}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{y} \hlstd{= ..density..),}
    \hlkwc{fill} \hlstd{=} \hlstr{"slategrey"}\hlstd{,} \hlkwc{color} \hlstd{=} \hlstr{"white"}\hlstd{,} \hlkwc{binwidth} \hlstd{=} \hlnum{1}\hlstd{)} \hlopt{+} \hlkwd{xlim}\hlstd{(}\hlnum{10}\hlstd{,} \hlnum{50}\hlstd{)} \hlopt{+} \hlkwd{facet_wrap}\hlstd{(}\hlopt{~}\hlstd{Mesh.Size)}
